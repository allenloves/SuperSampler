+ SS_SCMIRAudioFile {
	...
	standardizeFeature { |array, which=0, num=1, useglobal=false|
		...
		if(useglobal){
			...
			#mean,standarddeviation = SS_SCMIR.lookupGlobalFeatureNorms(which);
	...
	normalizeFeature { |array, which=0, num=1, useglobal=false|
		...
		if(useglobal){
			...
			#minval, maxval = SS_SCMIR.lookupGlobalFeatureNorms(which);
		} {
	...
	quantileFeature { |array, which=0, num=1, numquantiles=10, useglobal=false|
		...
		if(useglobal){
			...
			percentiles = SS_SCMIR.lookupGlobalFeatureNorms(which)[0];
