...
SS_SCMIR {
	...
	*setFrameHop {|hopsize|
		...
		if ((hopsize!= 1024) && (hopsize!=512)) {
			...
			"SS_SCMIR:setFrameHop: only hopsize of 1024 or 512 supported".postln;
	...
	*external {|command, scorerender=false, limit=2000|
		...
		if(thisThread.class==Routine) {
			...
			SS_SCMIR.waitOnUnixCmd(command, limit);
	...
	*saveGlobalFeatureNorms {|filename|
		...
		filename = filename ?? { (SS_SCMIR.getTempDir)++"globalfeaturenorms.scmirZ" };

		archive = SS_SCMIRZArchive.write(filename);
	...
	*loadGlobalFeatureNorms {|filename|
		...
		archive = SS_SCMIRZArchive.read(filename);
	...
	*findGlobalFeatureNorms {|filenamelist, featureinfo, normalizationtype=0,filestart=0,filedur=0,numquantiles=10,whichchannel|
		...
		filenamelist.do {|filename,j|
			...
			e = SS_SCMIRAudioFile(filename,featureinfo, normalizationtype,filestart,filedur);
	...
	*createARFF {|filename,numfeatures,classes|
		...
		filename = filename ?? {(SS_SCMIR.getTempDir)++"features.arff"};
	...
	*saveArchive {|filename,datatosave|
		...
		a = SS_SCMIRZArchive.write(filename);
