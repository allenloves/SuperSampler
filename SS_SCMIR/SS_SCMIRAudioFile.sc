...
SS_SCMIRAudioFile {
	...
	*newFromRange {|other,starttime=0.0,end|
	...
	initSCMIRAudioFileFromOther {|other,starttime=0.0,end|
		...
		var timeperhop= SS_SCMIR.hoptime;
	...
	initSCMIRAudioFile {|filename, fi, normtype=0, start=0, dur=0|
		...
		if (valid=SS_SCMIR.soundfile.openRead(filename),{

			loadstart = (start*(SS_SCMIR.soundfile.numFrames)).asInteger;

			if(loadstart<0 || (loadstart>=(SS_SCMIR.soundfile.numFrames))) {
				loadstart=0;
			};

			loadframes= if(dur==0) {SS_SCMIR.soundfile.numFrames} {

				loadtemp = dur*(SS_SCMIR.soundfile.sampleRate);

				if((loadstart + loadtemp) > (SS_SCMIR.soundfile.numFrames)) {

						loadtemp = SS_SCMIR.soundfile.numFrames- loadstart;
			...
			duration=  loadframes/SS_SCMIR.soundfile.sampleRate; //SCMIR.soundfile.numFrames/SCMIR.soundfile.sampleRate;
			numChannels = SS_SCMIR.soundfile.numChannels;

			sourcepath = filename;

			sourcedir = sourcepath.dirname;

			if(SS_SCMIR.tempdir.isNil,{
				analysispath = sourcepath.dirname ++ "/";
				},{
				analysispath = SS_SCMIR.tempdir;
			});
	...
	*resolveFeatures {|input, featurehop, featureinfo|
		...
		var fftsizeonset = SS_SCMIR.framehop; //512 or 1024; //should really be 512 with 256 overlap, but need to conform to general frame size choice
	...
	frameStartTimes {
		var featurehop = SS_SCMIR.framehop;
		var hoptime = featurehop/SS_SCMIR.samplingrate;
	...
	frameTimes {
		var featurehop = SS_SCMIR.framehop;
		var start = 4096/SS_SCMIR.samplingrate;
		var hoptime = featurehop/SS_SCMIR.samplingrate; //if (featurehop == 1024) {1024}{};
		var frametimes;
...
//		var fftsizeonset = SCMIR.framehop; //512 or 1024; //should really be 512 with 256 overlap, but need to conform to general frame size choice
		var featurehop = SS_SCMIR.framehop; //1024; //measurement about 40Hz
		...
		//	var mfccfft, chromafft, specfft, onsetfft;
			...
			#features, trig = SS_SCMIRAudioFile.resolveFeatures(input,featurehop,featureinfo);
		...
		Score.recordNRTSS_SCMIR(score,SS_SCMIR.nrtanalysisfilename,SS_SCMIR.nrtoutputfilename, nil,44100, "WAV", "int16", serveroptions); // synthesize
		...
		file = SS_SCMIRFile(analysisfilename,"rb");
...
//		};
	...
	save { |filename|
		...
		a = SS_SCMIRZArchive.write(filename);  //analysisfilename

		//future proofing, works as long as all have getters
		instancevars = SS_SCMIRAudioFile.instVarNames;
	...
	load { |filename|
		...
		a = SS_SCMIRZArchive.read(filename);
