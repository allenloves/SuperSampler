+ Sampler {
		*initClass{
		StartUp.add({
			...
			SynthDef(\expand, {arg buf=0, expand=1, dur=1, rate=1, amp=1, pan=0, out=0, grainRate=20, grainDur=0.15, startPos = 0, panSpread = 0.1;
				...
				var position = Line.kr(start: startPos/BufDur.kr(buf), end: 1, dur:  ((BufDur.kr(buf) - startPos) * expand));  //dur:  ((BufDur.kr(buf) - startPos) * expand));
